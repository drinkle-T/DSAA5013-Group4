...
def main_worker(gpu, ngpus_per_node, args, model_teacher, model_verifier, ipc_id_range):
    ...
    if not load_tag:
        train_dataset = torchvision.datasets.CIFAR100(root=args.train_data_path, train=True, download=True,
    ...
    counter = 0
    for zz in range(0, total_number, batch_size):  # 9[6 97 98 99] * 10
...
def main_syn():
    ...
    parser.add_argument('--train-data-path', type=str, default='./cifar-100/train',
    ...
    for name in aux_teacher:
        ...
        checkpoint = torch.load(
            os.path.join(args.pre_train_path, "CIFAR-100", name, f"squeeze_{name}.pth"),
            map_location="cpu")
